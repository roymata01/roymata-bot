...
function Avatar({ contact }: { contact: Contact }) {
  ...
  if (contact.avatar_url) {
    ...
    return <img src={contact.avatar_url} alt={label} className="h-9 w-9 shrink-0 rounded-full border-2 border-black object-cover" />;
  }
  return (
...
export function ConversationListItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-start gap-3 border-b border-black/10 px-4 py-3 text-left transition ${
