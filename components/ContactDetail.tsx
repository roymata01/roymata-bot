    ...
    <div className="mx-auto flex max-w-xl flex-col gap-5">
      <div className="flex items-center gap-3">
        {contact.avatar_url ? (
          ...
          <img src={contact.avatar_url} alt="" className="h-14 w-14 rounded-full border-2 border-black object-cover" />
        ) : null}
        <div>
          <h2 className="text-xl font-bold">{contact.display_name || contact.phone || contact.external_id}</h2>
          <div className="mt-1 flex items-center gap-2">
            <ChannelBadge channel={contact.channel} />
            {contact.phone && <span className="text-sm text-neutral-600">{contact.phone}</span>}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-lg border-2 border-black bg-white p-3">
          <p className="text-xs text-neutral-500">Primer contacto</p>
          <p className="font-medium">{new Date(contact.first_contact_at).toLocaleString("es-MX")}</p>
        </div>
        <div className="rounded-lg border-2 border-black bg-white p-3">
          <p className="text-xs text-neutral-500">Último contacto</p>
      ...
      <section className="rounded-2xl border-2 border-black bg-white p-4">
        ...
        <div className="mt-3 flex items-center gap-3">
          <button onClick={saveContact} className="rounded-lg border-2 border-black bg-black px-4 py-1.5 text-sm font-semibold text-white">
            Guardar
          </button>
          {savedAt && <span className="text-sm font-medium text-emerald-700">Guardado ✓</span>}
        </div>
      </section>
      ...
      <section className="rounded-2xl border-2 border-black bg-white p-4">
        ...
        {conversations.map((conv) => (
          <div key={conv.id} className="border-b border-black/10 py-2 text-sm last:border-0">
        ...
        {conversations.length === 0 && <p className="text-sm text-neutral-500">Sin conversaciones.</p>}
      </section>
    </div>
  );
}
